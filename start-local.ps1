...
Write-Host "───────────────────────────────────────────────────────"
Set-Location solver
if (Test-Path "target\release\brightsky.exe") {
...
Start-Job -Name "RustSolver" -ScriptBlock {
    Set-Location $args[0]
    & ".\target\release\brightsky.exe" 2>&1 | Tee-Object -FilePath ".\logs\rust-solver.log"
...
Write-Host ""
Write-Host "Monitoring profit (checking every 30 seconds)..."
