...
pub use core::process_manager::AppState;
pub use commands::{start_solver, stop_solver, get_solver_status, get_logs};
pub use commands::{get_readiness_status, run_readiness_check};
pub use commands::{set_user_role, get_user_role, complete_wizard, is_wizard_completed, set_exposure_limit, get_exposure_limit, can_start_stage, get_guru_defaults};
...
// custom setup code
    .setup(|app| {
      println!("Allbright Desktop starting...");
      println!("Workflow stages: dev -> simulation -> paper-trading -> shadow -> live-simulation -> canary -> live");
...
// Autostart plugin - auto-launch on system boot
#[cfg(not(any(target_os = "android", target_os = "ios")))]
      {
        // Autostart plugin - auto-launch on system boot (simplified for v2.6.0 compatibility)
        app.handle()
