...
impl PathCache {
    ...
    // Helper: Calculate profitability score for eviction priority with overflow protection
    fn calculate_profitability_score(opportunity: &ArbitrageOpportunity) -> f64 {
        ...
        // Clamp log_weight to prevent overflow/underflow in exp()
        let clamped_log_weight = opportunity.log_weight.max(-20.0).min(20.0);
        let exp_result = (-clamped_log_weight).exp();

        // Ensure result is finite and within reasonable bounds
        if exp_result.is_finite() && exp_result > 0.0 {
            exp_result.min(1e10) // Cap at reasonable maximum
        } else {
            0.0 // Default to neutral score on math errors
        }
    }
