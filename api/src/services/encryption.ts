...
export class EnvEncryption {
  ...
  constructor(secretKey: string) {
   ...
   encrypt(plaintext: string): string {
     ...
     const aesGcm = new (aesjs.ModeOfOperation as any).gcm(this.key, iv);
     const textBytes = (aesjs as any).utils.utf8.toBytes(plaintext);
   ...
   decrypt(encryptedBase64: string): string {
     ...
     const aesGcm = new (aesjs.ModeOfOperation as any).gcm(
       this.key,
       new Uint8Array(iv)
     );
     const decryptedBytes = aesGcm.decrypt(new Uint8Array(encrypted));
     return (aesjs as any).utils.utf8.fromBytes(decryptedBytes);
   }
}
